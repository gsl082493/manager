...
import com.kewei.manager.dao.BaseAdminPermissionMapper;
import com.kewei.manager.dao.BaseAdminRoleMapper;
import com.kewei.manager.dto.AdminRoleDTO;
...
public class AdminRoleServiceImpl implements AdminRoleService {
    ...
    @Override
    public Map<String, Object> insertRole(BaseAdminRole role) {
        Map<String,Object> data = new HashMap();
        try {
            role.setCreateTime(DateUtils.getCurrentDate());
            role.setUpdateTime(DateUtils.getCurrentDate());
            role.setRoleStatus(1);
            int result = baseAdminRoleMapper.insertRoles(role);
            if(result == 0){
                data.put("code",0);
                data.put("msg","新增角色失败");
                logger.error("新增角色失败");
